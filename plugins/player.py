...
async def add_to_playlist(_, message: Message):
    if Config.ADMIN_ONLY == "Y":
        ...
        if message.from_user.id not in admins:
            await message.reply_sticker("CAACAgEAAx0CSUIogQACBVVhTuegPT38gqOeaf-HSFGbjZPabAACOAIAAh8QeEZ14bvkMKKfeiEE")
            return
    type=""
    yturl=""
    ysearch=""
...
async def notforu(_, m: Message):
    await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgEAAx0CSUIogQACBWVhTuexxxeTiVCPykrdpHZNK3qOcwACFgIAAs3WeEa3944tRZ_gKyEE", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/Thekumikosetsuko')]]))
...
async def not_chat(_, m: Message):
    buttons = [
        [
            InlineKeyboardButton('💬Support chat', url='https://github.com/subinps/VCPlayerBot'),
